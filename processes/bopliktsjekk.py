...
from processes.utils.boplikt_db import bygg_boplikt_resultat, sjekk_boplikt, sjekk_kommune_boplikt
...
class BopliktSjekkProcessor(BaseProcessor):
    ...
    def execute(self, data, outputs=None):
        ...
        if not kommune_med_boplikt:
            LOGGER.info(
                "Kommune %s har ikke boplikt, returnerer nei", kommunenummer
            )
            return "application/json", {"boplikt": "nei"}

        if all(not kommune["delvis_boplikt"] for kommune in kommune_med_boplikt):
            LOGGER.info(
                "Kommune %s har full boplikt, returnerer ja", kommunenummer
            )
            return "application/json", bygg_boplikt_resultat("ja", kommune_med_boplikt[0])
